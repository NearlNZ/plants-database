<?php
    ...
    if($plantResult->num_rows > 0){
        $response->status = 'warning';
        $response->title = 'เกิดข้อผิดพลาด';
        $response->text = 'ชื่ิอพืชนี้ได้ทำการลงทะเบียนไปแล้ว';
    ...
    if($plantTags != Null){
        //Remove current plant tags
        $sql = "DELETE FROM tag_lists
                WHERE plantID = ?;";
        $stmt = $database->stmt_init(); 
        $stmt->prepare($sql);
    ...
    $sql = "UPDATE plants
            SET plantName = ?, commonName = ?, otherName = ?, scientificName = ?, familyName = ?, 
    ...
    $stmt = $database->stmt_init(); 
    $stmt->prepare($sql);
    $stmt->bind_param('ssssssssssssss',     $plantName, $commonName, $otherName, $scientificName, $familyName, 
                                            $plantTrunk, $plantLeaf, $plantFlower, $plantFruit, $plantCultivation, $plantPropagation,
    ...
    if($stmt->execute()){
        ...
        $response->text = 'แก้ไขข้อมูลพืชสำเร็จ';
